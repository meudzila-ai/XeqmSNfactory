...
class App:
    def __init__(self, root):
        ...
        self.max_in_peers = tk.StringVar(value="5")
    ...
    def setup_ui(self):
        # =========================================================================
        # 1. TOP CONTAINER & COLUMN LAYOUT CONFIGURATION
        # =========================================================================
        ...
        # =========================================================================
        # 2. LEFT PANEL: CONFIGURATION & SETTINGS
        # =========================================================================
        ...
        # -------------------------------------------------------------------------
        # 2.1 DEPLOYMENT DIRECTORY SECTION
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # 2.2 FACTORY SETTINGS SECTION (IN PEERS, OUT PEERS, NODES COUNT, WATCHDOG)
        # -------------------------------------------------------------------------
        ...
        # --- 1. MAX OUT PEERS CONTROL (ROW 0) ---
        tk.Label(settings_inner, text="Max Out Peers:", fg=TEXT_COLOR, bg=BG_COLOR, font=("Arial", 9, "bold"), anchor="w").grid(row=0, column=0, sticky="w", pady=(2, 2))
        
        peers_container = tk.Frame(settings_inner, bg=BG_COLOR)
        peers_container.grid(row=1, column=0, sticky="w", pady=(0, 6))
        ...
        # --- 2. MAX IN PEERS CONTROL (ROW 2) ---
        tk.Label(settings_inner, text="Max In Peers:", fg=TEXT_COLOR, bg=BG_COLOR, font=("Arial", 9, "bold"), anchor="w").grid(row=2, column=0, sticky="w", pady=(2, 2))
        
        in_peers_container = tk.Frame(settings_inner, bg=BG_COLOR)
        in_peers_container.grid(row=3, column=0, sticky="w", pady=(0, 6))
        
        self.in_peers_entry = tk.Entry(in_peers_container, textvariable=self.max_in_peers, bg=SECONDARY_BG, fg=TEXT_COLOR, insertbackground=TEXT_COLOR, bd=1, relief="flat", font=("Arial", 9, "bold"), width=12, justify="center")
        self.in_peers_entry.pack(side="left")
        ToolTip(self.in_peers_entry, "Controls incoming connections (Set 0 to prevent port overload).")

        # --- 3. NODES COUNT CONTROL (ROW 4) ---
        tk.Label(settings_inner, text="Nodes Count:", fg=TEXT_COLOR, bg=BG_COLOR, font=("Arial", 9, "bold"), anchor="w").grid(row=4, column=0, sticky="w", pady=(2, 2))
        ...
        self.entry_nodes.grid(row=5, column=0, sticky="w", pady=(0, 8))
        ...
        # --- 4. AUTO-WATCHDOG CHECKBOX (ROW 6) ---
        ...
        self.chk_watchdog.grid(row=6, column=0, sticky="w", pady=(4, 2))

        # -------------------------------------------------------------------------
        # 2.3 NODE MANAGER SELECTION GRID
        # -------------------------------------------------------------------------
        ...
        # =========================================================================
        # 3. RIGHT PANEL: NETWORK STATISTICS AND ACTION BUTTONS
        # =========================================================================
        ...
        # =========================================================================
        # 4. BOTTOM PANEL: REGISTRATION & CONSOLE LOGS
        # =========================================================================
        ...
        # Bind right-click menu to all input widgets
        for widget in [
            self.lbl_folder,
            self.peers_entry,
            self.in_peers_entry,  # Max In Peers entry added to context menu
            self.entry_nodes,
            self.entry_wallet,
            self.entry_stake,
            self.out_text,
    ...
    def deploy(self):
        # =========================================================================
        # 1. DEPLOYMENT LOCK & CONCURRENCY CHECK
        # =========================================================================
        ...
        # =========================================================================
        # 3. NUMERIC INPUT & PEERS VALIDATION
        # =========================================================================
        try:
            nodes_count_int = int(self.nodes_count.get())
            
            # Validate Outgoing Peers
            ...
            in_peers_str = self.max_in_peers.get().strip()
            in_peers_val = 0 if not in_peers_str else int(in_peers_str)
        except ValueError:
        ...
        # =========================================================================
        # 6. ASYNC TASK EXECUTION
        # =========================================================================
        current_ip = Network.get_public_ip()
        self.logger.log(
            f"🚀 Deploying nodes... IP: {current_ip} | "
            f"Max Out Peers: {peers_val} | Max In Peers: {in_peers_val}"
        )
    ...
    def hard_restart_docker(self):
        if messagebox.askyesno("Docker", "Emergency restart Docker Desktop?"):
            self.logger.log("🔄 Shutting down Docker...")
            
            def _restart_task():
                try:
                    # 1. Kill all Docker processes (GUI, backend, engine)
                    ...
                    # 2. Launch directly via standard executable path
                    docker_path = r"C:\Program Files\Docker\Docker\Docker Desktop.exe"
                    if os.path.exists(docker_path):
                        os.startfile(docker_path)
                        self.logger.log("🚀 Docker Desktop restarted successfully.")
                    else:
                        # Fallback using protocol handler
                        os.startfile("docker-desktop://")
                        self.logger.log("🚀 Docker restart command sent (via protocol).")
                except Exception as e:
                    self.logger.log(f"❌ Error restarting Docker: {e}")
