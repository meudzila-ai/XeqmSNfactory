...
class Controller:
    ...
    def deploy_nodes(self, path, selected_indices, public_ip, max_out_peers=16):
        """Generates compose file and deploys selected docker nodes."""
        self.log(f"Starting deployment for {len(selected_indices)} nodes...")
        if DockerManager.create_compose(path, selected_indices, public_ip, max_out_peers):
